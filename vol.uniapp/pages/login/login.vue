<template>
	<view class="login-container">
		<view class="login-title">請登入</view>
		<view class="login-item">
			<u--input v-model="userInfo.userName" placeholder="請輸入帳號" prefixIcon="account" shape="circle"
				border="bottom" clearable prefixIconStyle="font-size: 22px;color: #909399"></u--input>
		</view>
		<view class="login-item">
			<u--input v-model="userInfo.password" type="password" placeholder="請輸入密碼" prefixIcon="lock-open"
				shape="circle" border="bottom" clearable prefixIconStyle="font-size: 22px;color: #909399"></u--input>
		</view>
		<view class="login-item login-code">
			<u--input v-model="userInfo.verificationCode" placeholder="請輸入驗證碼" prefixIcon="scan" shape="circle"
				border="bottom" clearable prefixIconStyle="font-size: 22px;color: #909399"></u--input>
			<image class="img" @click="getVierificationCode" :src="codeSrc"></image>
		</view>
		<view class="login-btn">
			<u-button @click="login" :loading="loading" :loadingText="loading?'登錄中..':''"
				:customStyle="{'border-radius': '10rpx'}" size="large" type="primary" text="登錄">
			</u-button>
		</view>
		<view style="margin-top: -20rpx;">
			<vol-alert type="primary">
				<view>演示帳號：admin666 密碼：123456</view>
				<view>本地帳號：admin &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;密碼：123456</view>
			</vol-alert>
		</view>
		<u-divider text="其他方式登入"></u-divider>
	...
	export default {
		...
		methods: {
			login() {
				...
				this.loading = true;
				this.http
					.post("api/user/login", this.userInfo, "正在登錄....")
					.then((result) => {
						if (!result.status) {
							this.loading = false;
