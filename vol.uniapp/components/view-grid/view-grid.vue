		...
		<view class="sort-form-popup">
			<u-popup @touchmove.prevent :zIndex="9999999" :show="sortSelectModel" @close="sortSelectModel=false;">
				<view class="vol-action-sheet-select-container" style="max-height:400px">
					<view class="vol-action-sheet-select-title">請選擇排序
					</view>
		...
		<u-popup @touchmove.prevent :zIndex="999999" :show="model" @close="model=false">
			<view class="vol-action-sheet-select-container" :style="{'max-height':(maxHeight+'px')}">
				<view class="vol-action-sheet-select-title">
					{{buttons.length?(currentAction=='Add'?'新增':'編輯'):'基本信息'}}
	...
	export default {
		...
		data() {
			return {
				...
				rowIndex: false, //是否顯示table的行號
				load: true, //是否默認加載table表格數據
				height: 0, //當前table/列表高度
				direction: "list", //頁面顯示方向"horizontal"//list
				titleField: "", //如果table表格屬性direction是以list顯示，可以指定第一個標題
				fabButtons: [], //浮動按鈕、查詢、刷新、添加
				currentAction: 'Add', //當前操作狀態
				currentRow: {}, //當前編輯的行
				maxHeight: 0,
				model: false, //新建編輯彈出框
				showFabButons: true,
				showButtons: true, //是否顯示浮動按鈕
				textInline: true, //超出是否換行顯示
				...
				permission: [],
				buttons: [], //新建編輯彈出框中的按鈕
				hasEditpermission: false, //是否有編輯、新建權限
				searchModel: false, //搜索彈出框
				sortField: '',
				sortName: '',
				orderType: 'desc',
				sortSelectModel: false, //排序彈出框
				tableUrl: "", //table加載的url地址
				tableAction: "", //指定表名的權限
				showDel: false,
				isCreated: false,
				labelWidth: 80, //編輯彈出框表單標籤的寬度
				ck:false //設置顯示checkbox，只有水平(table)顯示類型時才生效
			}
		},
		methods: {
			onInited() {
			...
			editGirdFormOnChange(field, value,item) { //編輯新建表單的select、日期組件選擇事件
				this.editFormOnChange && this.editFormOnChange(field, value,item)
			},
			searchGridFormOnChange(field, value,item) { //查詢表單的select、日期組件選擇事件
				this.searchFormOnChange && this.searchFormOnChange(field, value,item)
			},
			getSearchItem(field) { //獲取查詢的參數
				let data = this.searchFormOptions.find(x => {
					return x.field == field
				});
				return (data || {}).type
			},
			getSearchParameters() { //獲取查詢參數
				let query = {
					wheres: []
				};
			...
			loadGridTableBefore(param, callback) { //查詢前
			...
			loadGridTableAfter(data, callback) { //查詢後
			...
			confirmDel() {
				...
				this.http.post(url, [key], true).then(result => {
					...
					this.model = false;
					//刷新頁面，如果是編輯不刷新頁面，直接更新表格數據(但小程序上有問題)
					this.refresh();
			...
			inputConfirm(field, e) { //input回車事件
				console.log(field)
			}
		},
		async created() {
