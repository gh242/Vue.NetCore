	...
	<view class="vol-table" :class="className">
		...
		<view v-else-if="direction=='horizontal'">
			...
			<view class="vol-table-body">
				<u-empty mode="list" v-if="!rowsData.length&&page>0" text="無數據"
				...
				<view v-if="hasSummary" :key="rowindex" class="vol-table-body-rows vol-table-summary"
					...
					<view class="cell-index" v-if="index">合計</view>
		...
		<u-overlay :opacity="0" :show="showOverlay" @click="showOverlay = false">
			<view class="loading-warp">
				<u-loading-icon text="加載中..." textSize="16"></u-loading-icon>

				<!-- 		<view class="loading-warp-msg" @tap.stop>正在加載</view> -->
			</view>
		</u-overlay>
	</view>
</template>
...
<script>
	export default {
		...
		props: {
			...
			titleField: { //如果direction是以list顯示，可以指定第一個標題
				type: String,
				default: ""
			},
			height: {
			...
			ck: { //設置顯示checkbox，只有水平(table)顯示類型時才生效
				type: Boolean,
				default: false
			},
			columns: {
