	...
	<view>
		...
		<view :style="{padding:padding+'rpx',display:item.hidden?'none':''}"
					...
					<view style="flex:1;">
						<view style="font-size:15px;">
							<input placeholder-style="color:rgb(192 196 204);font-size:15px;" type="number"
								...
								:placeholder="item.placeholder||('請輸入'+item.title)"></input>
						</view>
					</view>
				</view>
				<text style="margin:0 0rpx;">-</text>
			...
			<view class="f-form-content" v-else-if="item.type=='textarea'">
				<textarea :focus="item.focus" :ref="item.field" auto-height style="width: 100%;padding-right: 8rpx;"
					...
					:placeholder="item.placeholder||('請輸入'+item.title)"></textarea>
			</view>
			<!-- 	 -->
			...
			<view class="f-form-content" v-else>
				<input :focus="item.focus" placeholder-style="color:rgb(192 196 204);font-size:15px;" type="text"
					...
					:ref="item.field" :placeholder="item.placeholder||('請輸入'+item.title)"></input>
			</view>
		...
		<u-popup @touchmove.prevent class="form-popup" :zIndex="999999" :show="actionSheetModel"
			...
			<view class="vol-action-sheet-select-container" :style="{'height':(popupHeight+'px')}">
				<view class="vol-action-sheet-select-title">請選擇{{actionSheetCurrentItem.title}}
					<text class="vol-action-sheet-select-confirm" @click="actionConfirmClick">確定</text>
				</view>
				<!-- 	超過10個下拉框選項默認開啟搜索 -->
				<!-- 	 -->
				<view v-if="showFilter" class="vol-action-sheet-select-filter">
					<u-search @custom="searchText=''" placeholder="請輸入關鍵字搜索" :showAction="true" actionText="清除"
					...
					<!-- 	<view style="padding-left:20rpx;flex:1;font-size: 22px;color: #909399;background: white;">
						<u--input placeholder="請輸入關鍵字搜索" v-model="searchText">
						</u--input>
					</view>
		...
		<!--  樹形級聯組件 -->
		<vol-tree ref="cascader" :data="actionSascaderCurrentItem.data" :title="'請選擇'+actionSascaderCurrentItem.title"
	...
	export default {
		...
		data() {
			return {
				...
				popupHeight: 0,
				pickerValue: '',
				pickerModel: false, //日期組件
				pickerCurrentItem: {}, //當前選項
				pickerCurrentRangeIndex: 0,
				actionSheetModel: false,
				actionSascaderCurrentItem: {
					title: "",
					field: '',
					checkStrictly: false, //是否只能選擇最後一個節點
					cancel: () => {},
					confirm: () => {},
					data: []
				},
				...
				actionSheetSelectValues: [], //當前選中的項
				numberModel: false,
				numberType: 'number',
				numberCurrentItem: {},
				imgFields: []
		...
		methods: {
			...
			showActionSheet(item) {
				if (item.type == 'cascader') {
					...
					this.actionSascaderCurrentItem.checkStrictly = item.checkStrictly || false; //是否只能選擇最後一個節點
			...
			validate() {
				...
				if (_option) {
					if (['date', 'datetime', 'month', 'checkbox', 'select', 'selectList', 'radio', 'switch'].indexOf(
							...
							.type) != -
						1) {
						this.$toast('請選擇' + _option.title, )
					} else {
						this.$toast(_option.title + '不能為空')
					}
					return false;
				}
				return true;
			...
			uploadFilePromise(url, apiUrl) {
				return new Promise((resolve, reject) => {
					let a = uni.uploadFile({
						url: this.http.ipAddress + apiUrl, // 僅為示例，非真實的接口地址
						filePath: url,
						name: 'fileInput',
						header: {
							"uapp": 1,
			...
			onCitySelect(res) {
				...
				console.log(res);
				this.lotusAddressData.visible = res.visible; //visible為顯示與關閉組件標識true顯示false隱藏
				//res.isChose = 1省市區已選 res.isChose = 0;未選
				if (res.isChose) {
					...
					//this.region = `${res.province}${res.city}${res.town}`; //region為已選的省市區的值
				}
			},
			showCitySheet(item) {
				this.cityItem = item;
				...
				this.lotusAddressData.townName = arr[2] || ''; //區
		...
		// #ifdef MP-WEIXIN
		// 小程序不要使用循環生成表單,否則這裡會死循環,與初始化的時候設置默認值有關,後面再處理
		watch: {
			inFormFields: {
				handler(val) {
