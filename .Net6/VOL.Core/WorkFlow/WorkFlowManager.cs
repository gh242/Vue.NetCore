...
namespace VOL.Core.WorkFlow
{
    public static class WorkFlowManager
    {
        ...
        public static void AddProcese<T>(T entity, bool rewrite = false, bool changeTableStatus = true, Action<T, List<int>> addWorkFlowExecuted = null) where T : class
        {
            ...
            //{
            //    if (setp.StepType == (int)AuditType.用戶審批)
            //    {
            ...
            ) where T : class
        {
            ...
            if (workFlow == null)
            {
                return webResponse.Error("未查到流程信息,請檢查數據是否被刪除");
            }
            ...
            if (currentStep == null)
            {
                return webResponse.Error($"未查到流程陈點[workFlow.CurrentStepId]信息,請檢查數據是否被刪除");
            }
            ...
            if (filterOptions != null)
            {
                ...
                if (!CheckAuditStatus(workFlow, filterOptions, currentStep, status))
                {
                    ...
                    {
                        if (filterOptions.AuditRefuse == (int)AuditRefuse.返回上一節點)
                        {
                            msg = "審批未通過,返回上一節點";
                        }
                    ...
                    else if (AuditStatus.駁回 == status)
                    {
                        if (filterOptions.AuditBack == (int)AuditBack.返回上一節點)
                        {
                            msg = "審批被駁回,返回上一節點";
                        }
        ...
        private static bool CheckAuditStatus(Sys_WorkFlowTable workFlow, FilterOptions filterOptions, Sys_WorkFlowTableStep currentStep, AuditStatus status)
        {
            ...
            if (filterOptions.AuditRefuse == (int)AuditRefuse.返回上一節點 || filterOptions.AuditBack == (int)AuditBack.返回上一節點)
            {
            ...
            else if (filterOptions.AuditRefuse == (int)AuditRefuse.流程重新開始 || filterOptions.AuditBack == (int)AuditBack.流程重新開始)
            {
                //重新開始
        ...
        private static void SendMail(Sys_WorkFlowTable workFlow, FilterOptions filterOptions, Sys_WorkFlowTableStep nextStep, VOLContext dbContext)
        {
            ...
            Task.Run(() =>
            {
                ...
                try
                {
                    string title = $"有新的任務待審批：流程【{workFlow.WorkName}】,任務【{nextStep.StepName}】";
                    MailHelper.Send(title, title, string.Join(";", emails));
                    msg = $"審批流程發送郵件,流程名稱：{workFlow.WorkName},流程id:{workFlow.WorkFlow_Id},步驟:{nextStep.StepName},步驟Id:{nextStep.StepId},收件人:{string.Join(";", emails)}";
                    Logger.AddAsync(msg);
                }
                catch (Exception ex)
                {
        ...
        private static List<int> GetAuditUserIds(int stepType, string nextId = null)
        {
            ...
            else if (stepType == (int)AuditType.部門審批)
            {
