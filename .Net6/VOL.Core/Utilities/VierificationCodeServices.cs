...
namespace VOL.Core.Utilities
{
    ...
    {        //驗證碼字體集合
        ...
        static VierificationCodeServices()
        {
            ...
            {
                fonts = new string[] { "Verdana", "Microsoft Sans Serif", "Comic Sans MS", "Arial", "宋體" };
            }
            else
            {
                fonts = new string[] { "Arial", "Arial", "宋體", "宋體" };
            }
        }
        ...
        public static string CreateBase64Image(string code)
        {
            ...
            {
                pen.Color = random.GetRandom(colors);//随機顏色索引值

                pen.Typeface = SKTypeface.FromFamilyName(random.GetRandom(fonts), 700, 20, SKFontStyleSlant.Italic);//配置字體
                ...
                canvas.DrawText(code.Substring(i, 1), point, pen);//繪制一個驗證字符
