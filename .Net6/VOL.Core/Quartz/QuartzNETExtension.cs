...
namespace VOL.Core.Quartz
{
    public static class QuartzNETExtension
    {
        ...
        public static IApplicationBuilder UseQuartz(this IApplicationBuilder applicationBuilder, IWebHostEnvironment env)
        {
            ...
            {
                Console.WriteLine($"作業啟動異常:{ex.Message + ex.StackTrace}");
            }
        ...
        public static async Task<object> AddJob(this Sys_QuartzOptions taskOptions, ISchedulerFactory schedulerFactory, IJobFactory jobFactory = null)
        {
            ...
            try
            {
                ...
                {
                    msg = $"添加作業失敗，作業:{taskOptions.TaskName},表達式不正確:{ taskOptions.CronExpression}";
            ...
            {
                msg = $"作業啟動異常:{taskOptions.TaskName},{ex.Message}";
                Console.WriteLine(msg);
                QuartzFileHelper.Error(msg);
        ...
        {
            return schedulerFactory.TriggerAction(JobAction.刪除, taskOptions);
        }
        ...
        {
            return schedulerFactory.TriggerAction(JobAction.暫停, taskOptions);
        }
            ...
            try
            {
                ...
                switch (action)
                {
                    ...
                    case JobAction.暫停:
                        ...
                        {
                            taskOptions.Status = (int)JobAction.暫停;
                        }
                        break;
                    case JobAction.開啟:
        ...
        /// 
        /// </summary>
        /// <param name="context"></param>通過作業上下文獲取作業對應的配置參數
        /// <returns></returns>
        ...
        /// </summary>
        /// <param name="taskOptions"></param>
        /// <param name="init">初始化的不需要判斷</param>
        /// <returns></returns>
        public static (bool, object) Exists(this Sys_QuartzOptions taskOptions, bool init)
        {
            if (!init && _taskList.Any(x => x.TaskName == taskOptions.TaskName && x.GroupName == taskOptions.GroupName))
            {
                return (false,
                    ...
                    {
                        status = false,
                        msg = $"作業:{taskOptions.TaskName},分組：{taskOptions.GroupName}已經存在"
                    });
            }
            return (true, null);
        }
        ...
        public static (bool, string) IsValidExpression(this string cronExpression)
        {
            try
            {
                ...
                return (date != null, date == null ? $"請確認表達式{cronExpression}是否正確!" : "");
            }
            catch (Exception e)
            {
                return (false, $"請確認表達式{cronExpression}是否正確!{e.Message}");
            }
        }
    }
