...
namespace VOL.Core.Dapper
{
    public class SqlDapper : ISqlDapper
    {
        ...
        public int AddRange<T>(IEnumerable<T> entities, Expression<Func<T, object>> addFileds = null, bool beginTransaction = true)
        {
            ...
            if (key == null)
            {
                throw new Exception("實體必須包括主鍵才能批量更新");
            }
            string[] columns;
            ...
            else
            {
                ...
                //2020.11.21修復sqlserver批量寫入主鍵類型判斷錯誤
        ...
        /// <summary>
        /// sqlserver使用的臨時表參數化批量更新，mysql批量更新待發開
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="entity">實體必須帶主鍵</param>
        ...
        /// <summary>
        ///(根據主鍵批量更新實體) sqlserver使用的臨時表參數化批量更新，mysql待優化
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="entities">實體必須帶主鍵</param>
        ...
        public int UpdateRange<T>(IEnumerable<T> entities, Expression<Func<T, object>> updateFileds = null, bool beginTransaction = false)
        {
            ...
            if (DBType.Name == DbCurrentType.MySql.ToString())
            {
                ...
                // throw new Exception("mysql批量更新未實現");
            }
            ...
            catch (Exception ex)
            {
                ...
                if (ex.Message.Contains("local data is disabled"))
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        Console.WriteLine($"開啟mysql日誌寫入異常:{e.Message}");
                    }
                }
        ...
        private string DataTableToCsv(DataTable table)
        {
            ...
            //列内容如存在半角逗號（即,）則用半角引號（即""）將該字段值包含起來。
            //列内容如存在半角引號（即"）則應替換成半角双引號（""）轉義，並用半角引號（即""）將該字段值包含起來。
            ...
            foreach (DataRow row in table.Rows)
            {
                for (int i = 0; i < table.Columns.Count; i++)
                {
                    ...
                    {
                        //centos系統里把datatable里的日期轉換成了10/18/18 3:26:15 PM格式
