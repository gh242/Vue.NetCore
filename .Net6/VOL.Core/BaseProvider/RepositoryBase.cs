...
namespace VOL.Core.BaseProvider
{
    public abstract class RepositoryBase<TEntity> where TEntity : BaseEntity
    {
        ...
        /// 執行事務
        /// </summary>
        /// <param name="action">如果返回false則回滾事務(可自行定義規則)</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// 通過條件查詢返回指定列的數據(將TEntity映射到匿名或實體T)
            ...
            bool saveChange = false) where Detail : class
        {
            ...
            return webResponse.OK("修改成功,明細" + message, entity);
        }
            ...
            bool delNotExist = false) where TDetail : class
        {
            ...
            list.ForEach(x =>
            {
                ...
                {
                    //獲取所有修改的key,如果從數據庫查來的key,不在修改中的key，則為刪除的數據
                    keys.Add(val);
            ...
            return $"修改[{editCount}]條,新增[{addCount}]條,刪除[{delCount}]條";
        }
        ...
        /// <param name="delList">是否連明細一起刪除</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// 注意List生成的table的列順序必須要和數據庫表的列順序一致
        /// </summary>
