...
namespace VOL.System.Controllers
{
    ...
    public partial class Sys_UserController
    {
        ...
        public IActionResult ReplaceToken()
        {
            ...
            try
            {
                //如果5秒内替換過token,直接使用最新的token(防止一個頁面多個並發請求同時替換token導致token錯位)
            ...
            catch (Exception ex)
            {
                ...
                responseContent.Error("token替換異常");
            }
            finally
            {
                _lockCurrent.TryRemove(UserContext.Current.UserId, out object val);
                string _message = $"用戶{userInfo?.User_Id}_{userInfo?.UserTrueName},({(responseContent.Status ? "token替換成功": "token替換失敗")})";
        ...
        [ApiActionPermission]
        //通過ObjectGeneralValidatorFilter校驗參數，不再需要if esle判斷OldPwd與NewPwd參數
        ...
        public IActionResult ModifyUserPwd(string password, string userName)
        {
            ...
            {
                return Json(webResponse.Error("參數不完整"));
            }
            if (password.Length < 6) return Json(webResponse.Error("密碼長度不能少於6位"));
