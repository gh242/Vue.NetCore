    ...
    <el-scrollbar style="height: 100%;">
      ...
      <div class="home-list">
        <div class="list-item" @click="itemClick(item,index)" v-for="(item, index) in list" :key="index">
          ...
          <div :class="[item.type == '審批' ? 'item-strengthen' : (item.type == '新增' ? 'item-new' : 'item-other')]">{{ item.type }}</div>
        </div>
      </div>
    </el-scrollbar>
  </div>
...
export default {
  ...
  setup(props) {
    const list = reactive([
      ...
      { name: '物資申請',  desc: '這裡是一些不可描述的信息...', type: "審批" },
      { name: '採購申請',  desc: '這裡是一些不可描述的信息...',type: "審批" },
      {
        name: '合同申請',
        desc: '這裡是一些不可描述的信息...',
        type: "審批"
