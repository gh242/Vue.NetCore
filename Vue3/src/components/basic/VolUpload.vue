...
export default {
  ...
  props: {
    ...
    fileInfo: {
      //用於接收上傳的文件，也可以加以默認值，顯示已上傳的文件，用戶上傳後會覆蓋默認值
      type: Array,
      default: () => {
        return [];
    ...
    img: {
      //圖片類型  img>excel>fileTypes三種文件類型優先級
      type: Boolean,
      default: false
    },
    excel: {
  ...
  data() {
    return {
      ...
      changed: false, //手動上傳成功後禁止重覆上傳，必須重新選擇
      model: true,
      files: [],
      bigImg: '',
  ...
  methods: {
    ...
    async uploadOSS() {
      await this.http.get('api/alioss/getAccessToken', {}, false).then(async (x) => {
        ...
        let client = new OSS({
          ...
          // 從STS服務獲取的臨時訪問密钥（AccessKey ID和AccessKey Secret）。
    ...
    async upload(vail) {
      ...
      this.http
        .post(this.url+paramText, forms, this.autoUpload ? '正在上傳文件' : '')
        .then(
          (x) => {
