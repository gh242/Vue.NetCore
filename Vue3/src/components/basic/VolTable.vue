<template>
  ...
  >
    ...
    <div class="message" v-show="loading">加載中.....</div>
    <el-table
    ...
    >
      ...
      >
        ...
        <template #default="scope">
          <!-- 2022.01.08增加多表頭，現在只支持常用功能渲染，不支持編輯功能(涉及到組件重寫) -->
          <el-table-column
            style="border: none"
          ...
          <template v-else>
            ...
            ></div>
            <!-- 2021.11.18修復table數據源設置為normal後點擊行$event缺失的問題 -->
            <div
  ...
  >
    ...
    <template #footer>
        <div style="text-align: center; ">
          ...
          <el-button type="primary" size="small" @click="saveUpload" >儲存</el-button >
        </div>
    </template>
  </VolBox>
...
export default defineComponent({
  ...
  //沒有原先的selection屬性了，看issue上使用select/selectall獲取
  //監聽數組長度，如果刪除了數據，現在只能被迫清除所有選中的行
  watch: {
    'tableData.length': {
  ...
  data() {
    return {
      fixed: false, //是固定行號與4checkbox
      clickEdit: true, //2021.07.17設置為點擊行結束編輯
      randomTableKey: 1,
      visiblyColumns: [],
      key: '',
      realHeight: 0,
      ...
      errorFiled: '',
      edit: { columnIndex: -1, rowIndex: -1 }, // 當前雙擊編輯的行與4列坐標
      editStatus: {},
      ...
      selectRows: [], //當前選中的行
      isChrome: false,
      //vol-table帶數據源的單元格是否啟用tag標籤(下拉框等單元格以tag標籤顯示)
      //2023.04.02更新voltable與4main.js
      useTag: true,
      currentRow:{},
      currentColumn:[],
      fileInfo:[],
  ...
  methods: {
    ...
    rowClick(row, column, event) {
      ...
      // 點擊其他行時，如果點擊的行與4正在編輯的行相同，保持編輯狀態
      if (this.clickEdit && this.edit.rowIndex != -1) {
        if (row.elementIndex == this.edit.rowIndex) {
          // 點擊的單元格如果不可以編輯，直接結束編輯
          // 2020.10.12修復結束編輯時，element table高版本屬性獲取不到的問題
        ...
        //當正在編輯，且點擊到其他行時，在原編輯的行結束編輯後，觸發新行的rowClick事件
        //正在編輯時，禁止出發rowClick事件
        if (this.edit.rowIndex == -1) {
          this.$emit('rowClick', { row, column, event });
        }
    ...
    rowEndEdit(row, column, event) {
      ...
      if (this.edit.rowIndex != -1) {
        //2022.06.26修復表格内容切換後行數不一致時不能編輯的問題
    ...
    load(query, isResetPage) {
      ...
      let status = true;
      // 合併查詢信息(包查詢分頁、排序、查詢條件等)
      if (query) {
        param = Object.assign(param, query);
      }
      /* 查詢前處理(如果需要查詢條件，實現組件方法loadBefore方法即可:
    ...
    formatter(row, column, template) {
      ...
      let source = column.bind.data.filter((x) => {
        ...
        // 2020.06.06修復單獨使用table組件時,key為數字0時轉換成文本失敗的問題
    ...
    getSelectFormatter(column, val) {
      ...
      for (let index = 0; index < valArr.length; index++) {
        ( column.bind.orginData&&column.bind.orginData.length
          ...
          :column.bind.data)
        .forEach((x) => {
          // 2020.06.06修復數據源為selectList時,key為數字0時不能轉換文本的問題
