<template>
  <vol-box :footer="false" v-model="model" :height="height" :width="width" :padding="0" :lazy="true" title="審核">
    ...
    <div class="audit-model-content" :style="{ height: height - 100 + 'px' }">
      ...
      <el-radio-group v-show="hasFlow" style="padding-left: 15px;" v-model="activeName" class="ml-4">
        <el-radio label="audit" size="large">審核</el-radio>
        <el-radio label="log" size="large">審核記錄</el-radio>
      </el-radio-group>
      <div v-show="activeName == 'audit' || !hasFlow" class="audit-content">
            ...
            <div v-for="(item, index) in workFlowSteps" :key="index">
              <div class="step-item" :class="{'step-item-ad':item.auditId||item.stepAttrType=='start'}" v-if="item.stepAttrType == 'start'">
                ...
                <div class="right-item">
                  ...
                  <div class="step-text">發起人：{{ item.creator }}</div>
                </div>
              </div>
              ...
              <div v-else :class="{ 'step-current': item.isCurrent }" class="step-item">
                <div class="left-item">
                  <div>審批時間</div>
                  <div class="left-date">{{ item.auditDate || '待審批' }}</div>
                </div>
                <div class="right-item">
                  <div class="step-line"></div>
                  <i class="step-circle"></i>
                  ...
                  <div class="step-text">審批人：{{ item.auditor }}</div>
                  <div class="step-text">
                    狀 態： {{ getAuditStatus(item.auditStatus) }}
                  </div>
                  <div class="step-text">備 註： {{ item.remark || '-' }}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="fx-right" :style="{ width: !hasFlow ? '100%' : '400px' }" v-if="isCurrentUser || !hasFlow">
          ...
          <div v-if="!hasFlow">
            <el-alert :title="'當前選中【' + rowLen + '】條記錄待審核..'" type="success" :closable="false" />
          </div>
          <div class="rd">
            <span>審批：</span>
          ...
          <el-input style="padding-top: 10px;" v-model="auditParam.reason" type="textarea"
            :autosize="{ minRows: 4, maxRows: 10 }" placeholder="請輸入備註..."></el-input>
          <div class="btn">
            <el-button type="primary" @click="auditClick" icon="Check">審批</el-button>
          </div>
        </div>
      </div>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const columns = reactive([
      ...
      { title: '審批人', field: 'auditor', width: 80 },
      { title: '審批結果', field: 'auditStatus', width: 70, bind: { data: [] } },
      { title: '審批時間', field: 'auditDate', width: 145 },
      { title: '備註', field: 'remark', width: 120 }
    ]);
