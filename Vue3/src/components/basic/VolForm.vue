		...
		remote
            ...
            >
              <el-date-picker
                ...
                :type="item.type"
                :placeholder="
                  item.placeholder ? item.placeholder : '請選擇' + item.title
                "
                :disabledDate="(val) => getDateOptions(val, item)"
                :value-format="getDateFormat(item)"
              >
            ...
            <el-input
              ...
              :placeholder="item.placeholder ? item.placeholder : item.title"
            />
            <!-- 2021.11.18修復el-input沒有默認enter事件時回車異常 -->
            <el-input
              :size="size"
              clearable
              :ref="item.field"
...
//表單驗證注意：每次驗證都必須執行callback,否則驗證不執行回調方法
  ...
  setup(props, context) {
    ...
    const initFormRules = (init) => {
      ...
      props.formRules.forEach((row, xIndex) => {
        ...
        row.forEach((item, yIndex) => {
          ...
          // 目前只支持select單選遠程搜索，remote遠程從後台字典數據源進行搜索，url從指定的url搜索
    ...
    const validate =async (callback) => {
      ...
      await volform.value.validate((valid) => {
        if (!valid) {
          appContext.config.globalProperties.$message.error('數據驗證未通過!');
          result = false;
  ...
  methods: {
    getColWidth(item) {
      ...
      this.formRules.forEach((row, xIndex) => {
        //2022.05.06 追加表單中隱藏的元素不參與4動態計算表單寬度
    ...
    // 是否為圖片文件等格式並對字段的轉換成數組：[{name:'1.jpg',path:'127.0.0.1/ff/1.jpg'}]
    ...
    convertFileToArray(item, formFields) {
      ...
      // 將以逗號隔開的文件分割成數組127.0.0.1/aa/1.jpg,將127.0.0.1/aa/2.jpg
    ...
    validatorPwd(ruleOption, value, callback) {
      ...
      if ((value + '').trim().length < 6) {
        return callback(new Error('密碼長度不能小於6位'));
      }
      callback();
    },
    convertArrayValue(data, val) {
      ...
      // 編輯多選table顯示
      //2023.04.20修復只讀為label時原數據被字典替換了的問題
    ...
    getText(formFields, item) {
      // 2019.10.24修復表單select組件為只讀的屬性時沒有綁定數據源
      ...
      //2021.03.31修復表單switch只讀時沒有轉換值的問題
      if (item.type == 'switch') {
        return text ? '是' : '否';
      }
    ...
    onRemoteChange(item, value) {
      // 第二次打開時，默認值成了undefined，待查viewgrid中重置代碼
    ...
    remoteSearch(item, formFields, val) {
      ...
      // 彈出框或初始化表單時給data設置數組默認值2
      // 2020.09.26修復遠程搜索自定義url不起作用的問題
      let url;
    ...
    getRule(item, formFields) {
      ...
      if (item.validator && typeof item.validator === 'function') {
        return {
          validator: (rule, val, callback) => {
            // 用戶自定義的方法，如果返回了值，直接顯示返回的值，驗證不通過
        ...
        return {
          validator: (rule, val, callback) => {
            ...
            if (
              ...
              return callback(
                new Error(item.type == 'img' ? '請上傳照片' : '請上傳文件')
              );
            }
            return callback();
          },
          ...
          validator: (ruleObj, value, callback) => {
            ...
            if (ruleObj.type == 'number') {
              if (!rule.number.test(value)) {
                ruleObj.message = ruleObj.title + '只能是整數';
                return callback(new Error(ruleObj.message));
              }
            } else {
              if (!rule.decimal.test(value)) {
                ruleObj.message = ruleObj.title + '只能是數字';
                return callback(new Error(ruleObj.message));
              }
            }
            if (
      ...
      if (inputTypeArr.indexOf(item.type) != -1) {
        ...
        if (item.min) {
          _rule.min = item.min;
          _rule.message = item.title + '至少' + item.min + '個字符!';
        }
        if (item.max) {
          return [
            _rule,
            {
              max: item.max,
              required: true,
              message: item.title + '最多' + item.max + '個字符!',
              trigger: 'blur'
            }
          ];
        }
      ...
      if (item.type == 'cascader') {
        return {
          ...
          validator: (rule, val, callback) => {
            ...
            if (!_arr || !_arr.length) {
              return callback(new Error('請選擇' + item.title));
            }
            return callback();
          }
        };
      ...
      if (
        ...
        if (_rule.max) {
          _rule.nax = item.max;
          _rule.message = '最多只能選擇' + item.max + '項';
        }
        return _rule;
      }
      return {};
...
.el-form-item ::v-deep(textarea) {
  font-family: 'Helvetica Neue', Helvetica, 'PingFang SC', 'Hiragino Sans GB',
    'Microsoft YaHei', '微軟雅黑', Arial, sans-serif !important;
}
